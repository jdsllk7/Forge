...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (isset($_POST['add']) && $_POST['add'] == 'add' || (isset($_POST['finish']) && $_POST['finish'] == 'finish')) {
    ...
    if (
      ...
      if ((isset($_SESSION["editActivities"]) && $_SESSION["editActivities"] == 1)) {
        $sql4 = "DELETE FROM created_pages WHERE user_id = $user_id AND page_name LIKE 'Activities'";
        $sql5 = "DELETE FROM activities WHERE user_id = $user_id";
      ...
      if (mysqli_query($conn, $sql)) {
        ...
        $data = mysqli_query($conn, "SELECT * from created_pages WHERE user_id = $user_id and page_name like 'Activities'");

        if (mysqli_num_rows($data) == 0) {
          if (mysqli_query($conn, $sql3)) {
            if (isset($_POST['finish']) && $_POST['finish'] == 'finish') {

              $data = mysqli_query($conn, "SELECT * from activities WHERE user_id = $user_id");
